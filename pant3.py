...
# 긴 형식으로 변환 (melt)
df_long = df.melt(id_vars='시도별', var_name='연도', value_name='생산량 (톤)')
...
# 시도 다중 선택
regions = df_long['시도별'].unique().tolist()
selected_regions = st.multiselect("🔍 비교할 시도를 선택하세요:", regions, default=regions[:3])

if selected_regions:
    df_filtered = df_long[df_long['시도별'].isin(selected_regions)]
    
    fig = px.line(df_filtered, x='연도', y='생산량 (톤)', color='시도별',
